...
void cPlayer::Init()
{
	...
	m_pObject->SetAlarmFunc([&](int Index)->void
	{
		...
		case 9:
		{
			if (m_BFGCharging != 0)
			{
				float Rot = DegAngle(m_pObject->GetPosition() + Vec2(0, 10), INPUT->GetMousePos());
				...
				{
					Part->m_Pos = OBJECT->GetPlayer()->GetPosition() + Vec2(0, 10) + RotateVec(Vec2(35, 0), DegAngle(m_pObject->GetPosition() + Vec2(0, 10), INPUT->GetMousePos()));
				});
	...
	GetComponent<cCollider>()->SetCollFunc([&](cObject* Other)->void
	{
		...
		case Tag::Item:
		{
			...
			else if (Other->GetName() == "Item_Chain")
			{
				m_ChainAmmo += 6;
				if (m_ChainAmmo > 100)
				{
			...
			else if (Other->GetName() == "Item_Rocket")
			{
				m_RocketAmmo += 2;
				if (m_RocketAmmo > 15)
				{
			...
			else if (Other->GetName() == "Item_Sniper")
			{
				m_SniperAmmo += 3;
				if (m_SniperAmmo > 10)
				{
					m_SniperAmmo = 10;
					SYSTEM->AddScore(1000);
				}
			}
			Other->Destroy();
...
void cPlayer::Update()
{
	...
	if (!m_Dead)
	{
		if (!m_InDash)
		{
			if (INPUT->KeyDown('W') & (m_pObject->GetPosition() + Vec2(0, -m_Speed) * TimeScale).y >= 20)
			{
				m_pObject->Translate(Vec2(0, -m_Speed) * TimeScale);
				Moved = true;
			}
			if (INPUT->KeyDown('S') & (m_pObject->GetPosition() + Vec2(0, -m_Speed) * TimeScale).y <= 700)
			{
				m_pObject->Translate(Vec2(0, m_Speed) * TimeScale);
				Moved = true;
			}
			if (INPUT->KeyDown('A') & (m_pObject->GetPosition() + Vec2(0, -m_Speed) * TimeScale).x >= 20)
			{
				m_pObject->Translate(Vec2(-m_Speed, 0) * TimeScale);
				Moved = true;
			}
			if (INPUT->KeyDown('D') & (m_pObject->GetPosition() + Vec2(0, -m_Speed) * TimeScale).x <= 980)
			{
				m_pObject->Translate(Vec2(m_Speed, 0) * TimeScale);
				Moved = true;
			}
			m_Speed = 5;
			if (INPUT->KeyPress(VK_SPACE) && Moved && m_DashAble)
			{
				m_InDash = true;
				m_DashAble = false;
				m_pObject->Alarm(4, 30);
				D3DXVec2Normalize(&m_DashDirection, &(m_pObject->GetPosition() - PrevPos));
				m_DashDirection *= 40;
		...
		if (INPUT->KeyDown(VK_LBUTTON) && m_AttackAble && m_Weapon != Wep_BFG)
		{
			float Rot = DegAngle(m_pObject->GetPosition() + Vec2(0, 10), INPUT->GetMousePos());
			cObject* a;
			switch (m_Weapon)
			...
			case Wep_Default:
			{
				...
				for (int i = -1; i <= 1; i++)
				{
					...
					a->AddComponent<cBullet>()->SetDamage(4);
			...
			case Wep_Plasma:
			{
				...
				if (m_PlasmaAmmo > 0)
				{
					...
					a->AddComponent<cBullet>()->SetDamage(30);
		...
		if (m_BFGCharging != 0)
		{
			...
			if (m_PlasmaAmmo - (10 + (int)m_BFGCharging) <= 0)
			{
				FireBFG();
			}
			if (m_BFGCharging >= 20)
		...
		if (INPUT->KeyPress(VK_LBUTTON) && m_Weapon == Wep_BFG)
		{
			if (m_PlasmaAmmo >= 10 && m_AttackAble)
			{
				m_BFGCharging = 0.14285714285714285714285714285715;
		...
		if (m_BFGCharging == 0)
		{
			if (INPUT->KeyDown(VK_RBUTTON) && m_MeleeAble)
			{
				m_MeleeAble = false;
				m_pObject->Alarm(8, 60);
				float Rot = DegAngle(m_pObject->GetPosition(), INPUT->GetMousePos());
			...
			if (INPUT->KeyPress('1'))
			{
				m_Weapon = Weapon::Wep_Default;
			}
			if (INPUT->KeyPress('2'))
			{
				m_Weapon = Weapon::Wep_Plasma;
			}
			if (INPUT->KeyPress('3'))
			{
				m_Weapon = Weapon::Wep_Chain;
			}
			if (INPUT->KeyPress('4'))
			{
				m_Weapon = Weapon::Wep_Rocket;
			}
			if (INPUT->KeyPress('5'))
			{
				m_Weapon = Weapon::Wep_Sniper;
			}
			if (INPUT->KeyPress('6'))
			{
				m_Weapon = Weapon::Wep_BFG;
			}
		}
	}
	else
...
void cPlayer::Render()
{
	if (!m_Dead)
	{
		...
		{
			IMAGE->CenterRender(IMAGE->Find("Turret_Medium1"), m_pObject->GetPosition() + Vec2(0, 10), RadAngle(m_pObject->GetPosition(), INPUT->GetMousePos()) + D3DXToRadian(90), Scale, Color);
			break;
		}
		case Weapon::Wep_Plasma:
		{
			IMAGE->CenterRender(IMAGE->Find("Turret_Medium2"), m_pObject->GetPosition() + Vec2(0, 10), RadAngle(m_pObject->GetPosition(), INPUT->GetMousePos()) + D3DXToRadian(90), Scale * 1.3, Color);
			break;
		}
		case Weapon::Wep_Chain:
		{
			IMAGE->CenterRender(IMAGE->Find("Turret_Large2"), m_pObject->GetPosition() + Vec2(0, 10), RadAngle(m_pObject->GetPosition(), INPUT->GetMousePos()) + D3DXToRadian(90), Scale * 1.2, Color);
			break;
		}
		case Weapon::Wep_Rocket:
		{
			IMAGE->CenterRender(IMAGE->Find("Turret_Small2"), m_pObject->GetPosition() + Vec2(0, 10), RadAngle(m_pObject->GetPosition(), INPUT->GetMousePos()) + D3DXToRadian(90), Scale * 1.5, Color);
			break;
		}
		case Weapon::Wep_Sniper:
		{
			IMAGE->CenterRender(IMAGE->Find("Turret_Small3"), m_pObject->GetPosition() + Vec2(0, 10), RadAngle(m_pObject->GetPosition(), INPUT->GetMousePos()) + D3DXToRadian(90), Scale * 1.5, Color);
			break;
		}
		case Weapon::Wep_BFG:
		{
			IMAGE->CenterRender(IMAGE->Find("Turret_Large1"), m_pObject->GetPosition() + Vec2(0, 10), RadAngle(m_pObject->GetPosition(), INPUT->GetMousePos()) + D3DXToRadian(90), Scale * 1.2, Color);
			break;
		}
		}
	}
...
void cPlayer::FireBFG()
{
	...
	a->GetComponent<cBullet_BFG>()->SetCharge(m_BFGCharging + 10);
	...
	m_PlasmaAmmo -= 10 + (int)m_BFGCharging;
	m_BFGCharging = 0;
